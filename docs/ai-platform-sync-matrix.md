...
| **Claude Code / Cowork** | Cloud-run, account-bound sessions; ephemeral containers | Yes (reads repo directly) | Yes (commits + PRs) | Implementation, patches, PRs | Treating the session as durable memory | Commit + push every session; durable state is the repo |
