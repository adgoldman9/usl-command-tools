...
$countsLines = ($counts.GetEnumerator() | ForEach-Object {
    "- $($_.Key): $($_.Value)"
}) -join "`r`n"
$cadPresenceText = if ($cadPresent) { "PRESENT" } else { "REVIEW_REQUIRED / not found at expected path" }
$driveLocalText = if ($driveLocal) { " at $(Protect-Path $driveLocalPath)" } else { "" }
$localSourcePresenceLines = ($sourceRows |
    Where-Object { $_.source_class -in @("LOCAL_ONEDRIVE_SOURCE","LOCAL_DESKTOP_SOURCE","LOCAL_DOCUMENTS_SOURCE") } |
    ForEach-Object { "- $($_.source_id): $($_.access_status) - $($_.source_path)" }
) -join "`r`n"
$cadCompletionText = if ($cadPresent) { "present" } else { "path review required" }
$cadHostText = if ($cadPresent) { "PRESENT (index-only)" } else { "REVIEW_REQUIRED" }
...
- Local presence: $cadPresenceText
...
Detected Google Drive local status: **$driveStatus**$driveLocalText
...
- CAD generator: index-only ($cadCompletionText).
...
Write-Host "CAD generator:         $cadHostText"
